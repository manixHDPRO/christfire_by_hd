import { apiGetStockItemUnits, apiPutStockItemUnits } from "@/lib/api";
...
export function StockItemUnitsSettingsPanel({
  readOnly,
  baseId,
}: {
  readOnly: boolean;
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    setLoadErr(false);
    const data = await apiGetStockItemUnits();
    setLoading(false);
    if (!data?.units) {
      setLoadErr(true);
      setRows([]);
  ...
  const addRow = () => {
    setRows((prev) => [...prev, { code: "", label: "", sortOrder: prev.length, active: true }]);
  };
  ...
  const save = async () => {
    ...
    for (const r of normalized) {
      ...
      if (!r.label) {
        setSaveErr("Le libellé est obligatoire pour chaque unité.");
        return;
      }
    }
    ...
    if (!normalized.some((r) => r.active)) {
      setSaveErr("Au moins une unité doit rester active.");
      return;
    }
    setSaving(true);
    const res = await apiPutStockItemUnits({ units: normalized });
    setSaving(false);
    if (res && "error" in res) {
      if (res.error === "unit_in_use" && res.code) {
        setSaveErr(`L’unité « ${res.code} » est encore utilisée par des articles. Réassignez-les avant suppression.`);
  ...
  return (
    ...
    >
      <div className="mb-4">
        <h2 className="font-display text-xl tracking-wide text-brand-cream/90">Unités article (inventaire)</h2>
        <p className="mt-1 text-xs text-white/40">
          Codes techniques pour le champ « Unité » des articles (logistique). Seules ces unités actives sont proposées
          à la création et à l’édition. Au moins une unité active. La suppression d’un code n’est possible que si aucun
          article ne l’utilise.
        </p>
        {loadErr ? (
          <p
            className="mt-2 rounded-lg border border-brand-orange/30 bg-brand-orange/10 px-3 py-2 text-xs text-brand-cream/95"
            role="alert"
          >
              ...
              rows.map((r, idx) => (
                <tr key={`${r.code || "new"}-${idx}`} className="border-b border-white/5 hover:bg-white/[0.02]">
                  <td className="px-4 py-2 align-top">
                    <label className="sr-only" htmlFor={`${baseId}-unit-code-${idx}`}>
                      Code
                    </label>
                    <input
                      id={`${baseId}-unit-code-${idx}`}
                      value={r.code}
                      readOnly={readOnly}
                      onChange={(e) => {
                      ...
                      placeholder="ex. caisse_24"
                    />
                  </td>
                  <td className="px-4 py-2 align-top">
                    <label className="sr-only" htmlFor={`${baseId}-unit-label-${idx}`}>
                      Libellé
                    </label>
                    <input
                      id={`${baseId}-unit-label-${idx}`}
                      value={r.label}
                      readOnly={readOnly}
                      onChange={(e) => {
                        const v = e.target.value;
